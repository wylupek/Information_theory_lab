...
def create2(code_size: float) -> dict:
    code_format = "0" + str(math.ceil(math.log2(code_size))) + "b"
    return {i: format(i, code_format)
            for i in range(int(code_size))}
...
def save2(filename: str, list_of_codes: list, codes: dict) -> None:
    with open(filename + ".bin", 'wb') as file:
        encoded_text = ba.bitarray()
        for code in list_of_codes:
            encoded_text.extend(ba.bitarray(codes[code]))
...
def decode2(bit_array: ba.bitarray, code, code_size: float):
    code_inv = {v: int(k) for k, v in list(code.items())}
    decoded_codes = []
    current_code = ""
...
if __name__ == '__main__':
    my_code_size = 2**12
    # my_code_size = float('inf')
    my_filename = "../data/lena.bmp"

    # Load -> Encode -> Save
    my_bit_array = load_file(my_filename)
    encoded_data = encode(my_bit_array, my_code_size)
    my_code = create2(my_code_size)
    # my_code = create(encoded_data)
    save2(my_filename, encoded_data, my_code)
    # save(my_filename, encoded_data, my_code)
    print("Loaded file len:", len(my_bit_array))

    # Load -> Decode -> Compare
    my_encoded_text_loaded = load2(my_filename)
    my_code_generated = create2(my_code_size)
    decoded_data = ''.join(decode2(my_encoded_text_loaded, my_code_generated, my_code_size))
...
norm_wiki_sample.txt        10537       6233+1          8974        6560
wiki_sample.txt             11630       7391+1          10441       7606
lena.bmp                    11255       10522+1         12186       11373
